...
//RAV
   EAVYCallSpawnCompatibilityCheck(ent);
//
...
//		orgtargetname = e->targetname;
				...
				if(findteam && lc > 0)
				{
					gi.dprintf("%i train chaining found.\n",lc);
					for(k = 0;k < lc; k++)
					{
...
void SpawnEntities (char *mapname, char *entities, char *spawnpoint)
{
	...
	while (1)
	{
		...
		if (ent != g_edicts)
		{
			if (deathmatch->value)
			{
				...
				if (Q_stricmp(ent->classname, "item_flag_team1") == 0 && !ctf->value)
				{
					G_FreeEdict (ent);	
					inhibit++;
					continue;
				}
				if (Q_stricmp(ent->classname, "item_flag_team2") == 0 && !ctf->value)
				{
					G_FreeEdict (ent);	
					inhibit++;
					continue;
	...
	else	// make sure CTF is enabled, since there is a flag
	{
		...
		if (ctf->value)
		{
			if(voosh->value)
			{
				gi.configstring (CS_STATUSBAR, rail_statusbar);
			}
			else
			{
...
void SP_worldspawn (edict_t *ent)
{
	...
	if (deathmatch->value)
	{
		if (voosh->value)
		{
			gi.configstring (CS_STATUSBAR, raildm_statusbar);
		}
		else
		{
	...
	{
		gi.soundindex("misc/headshot.wav");
		gi.soundindex("misc/headhunter.wav");
		gi.soundindex("misc/dominating.wav");
		gi.soundindex("misc/dominatingf.wav");
		gi.soundindex("misc/godlike.wav");
		gi.soundindex("misc/godlikef.wav");
		gi.soundindex("misc/humiliation.wav");
		gi.soundindex("misc/killingspree.wav");
		gi.soundindex("misc/killingspreef.wav");
		gi.soundindex("misc/multikill.wav");
		gi.soundindex("misc/multikillf.wav");
		gi.soundindex("misc/rampage.wav");
		gi.soundindex("misc/rampagef.wav");
		gi.soundindex("misc/doublekill.wav");
		gi.soundindex("misc/ultrakill.wav");
		gi.soundindex("misc/ultrakillf.wav");
		gi.soundindex("misc/monsterkill.wav");
		gi.soundindex("misc/mmmmonster.wav");
		gi.soundindex("misc/megakill.wav");
		gi.soundindex("misc/ludicrouskill.wav");
		gi.soundindex("misc/holyshitf.wav");
		gi.soundindex("misc/doublekill.wav");
		gi.soundindex("misc/doublekillf.wav");
	}
