...
void GetDate()
 {
	...
	ltime=localtime((unsigned long *)&gmtime);
	...
	if(day < 10 && month < 10)
		sprintf (sys_date,"0%1i-0%1i-%4i", month, day, year);
	else if(day < 10)
		sprintf (sys_date,"%2i-0%1i-%4i", month, day, year);
	else if(month < 10)
		sprintf (sys_date,"0%1i-%2i-%4i", month, day, year);
	else
		sprintf (sys_date,"%2i-%2i-%4i", month, day, year);
...
void GetTime()
 {
	...
	if((min < 10)&&(sec < 10))
	{
		sprintf (sys_time2,"%2i:0%i:0%i%s", hour, min, sec, ampm);
	}

	else if(min < 10)
	{
		sprintf (sys_time2,"%2i:0%i:%2i%s", hour, min, sec, ampm);
	}

	else if(sec < 10)
	{
		sprintf (sys_time2,"%2i:%2i:0%i%s", hour, min, sec, ampm);
	}

	else
	{
		sprintf (sys_time2,"%2i:%2i:%2i%s", hour, min, sec, ampm);
	}
		
	if (strcmp(sys_time, sys_time2) != 0)
	{
...
//	strcpy (time_left ,"/0");
	...
	if (sec < 0)
	{
		sprintf (time_left2 ,"00:00");	
	}
	else if (sec < 10)
	{
		sprintf (time_left2 ,"%i:0%i", min, sec);
	}
	else
	{
		sprintf (time_left2 ,"%i:%i", min,sec);
	}
...
//		mindist = ((light_comp->value/100) * tr.ent->light_level);	//JSW Comment - why use this? they are either there or they aren't,
	...
	if(runes->value)
	{
		if (rune_has_a_rune(ent))
		{
			for (i=RUNE_FIRST; i<=RUNE_LAST; i++)
			{
				...
				{
					if ((rune = FindItem(rune_namefornum[i])) != NULL && ent->client->pers.inventory[ITEM_INDEX(rune)])
...
char *tn_showHud (edict_t *ent)
{
	...
	//spec
	else if ((ent->client->resp.spectator != 0 || ent->client->pers.pl_state == 0)
		&& (ent->client->pers.motd == false))
	{
		if (ent->client->chase_mode == 0)
		{
			...
			if(ent->client->chase_target != NULL)
				j += sprintf (layout+j, va ("xv 0 yb -78 cstring \"%s\" ", ent->client->chase_target->client->pers.netname ));
	...
	if(ent->client->pers.pl_state != 0)
	{
		...
		{
			j += sprintf (layout+j, "xl 2 yb -50 string2 \"%s %s\" ",MOD ,MOD_VERSION);
			if(show_time->value)
			{
				j += sprintf (layout+j, "xl 2 yb -80 string2 \"Server Time\" ");
				j += sprintf (layout+j, "xl 2 yb -70 string \" %s\" ",sys_time);
			}
			if (ent->is_muted)
				j += sprintf (layout+j, "xl 2 yb -34 string2 \"You are Muted\" ");
			if (notfairBLUE)
			...
			if (ent->client->pers.isop == 1)
				j += sprintf (layout+j, "xl 2 yb -42 string2 \"Operator Level %d\" ", ent->client->pers.oplevel);
		}
	}
	//id dissabling added 12-14-99 raven

	if ((!ent->client->chase_target) && (ent->client->pers.db_id))
		j += sprintf (layout+j, tn_id (ent));
