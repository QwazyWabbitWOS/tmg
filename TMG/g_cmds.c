...
if (!G_EntExists(ent))
 return 0;
	 ...
	 if  (ent->client->resp.ctf_team < 3 && ent->client->resp.ctf_team > 0)
...
*/
	...
	if(ctf->value)
	{
		...
		else if (ent->client->pers.pl_state == 0)
		{
	...
	for (j = 1; j <= game.maxclients; j++)
	{
		...
		if (team)
		{
			if (other->client->pers.pl_state != 0)// || !OnSameTeam(ent, other))
				continue;
		}
...
=================
*/
void vote (edict_t *ent, char *command,char *command1, float timenow);
...
*/
	...
	//RAVEN
		...
		||(Q_stricmp (cmd, "chase")== 0)||(Q_stricmp (cmd, "chasecam")== 0)
		|| (Q_stricmp (cmd, "Observe")== 0)||(Q_stricmp (cmd, "Observer")== 0)
		&& (ent->client->pers.pl_state != 0)&&(!ent->client->pers.pl_state !=5))
	{
	...
	{
		if ((ent->solid != SOLID_NOT &&
			ent->deadflag == DEAD_NO)&&(ent->client->pers.pl_state != 5)&&
	...
	*/
	...
	{
		if(atoi(gi.argv(1)) != 1 && ent->client->pers.pl_state < 5)
		{
			ent->client->newweapon = NULL;
			ChangeWeapon (ent);
			ent->client->ps.gunindex = 0;
