...
#if defined(linux)
		sprintf(name,"%s/%s/%s/nav/%s.nav",basedir->string, game_dir->string, cfgdir->string,level.mapname);
#else
		sprintf(name,"%s\\%s\\%s\\nav\\%s.nav",basedir->string, game_dir->string, cfgdir->string,level.mapname);
#endif
...
void SpawnCommand(int i)
{
	...
	if(chedit->value){ 
		gi.cprintf(NULL,PRINT_HIGH,"Can't spawn.");return;}
