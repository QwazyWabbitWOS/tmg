...
*/
/**
 Find item based on pickup_name.
 Return the pointer to the item object.
 */
...
void Touch_Item (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
		...
		|| (ctf->value && other->client->resp.ctf_team < 1))
	{
		...
		other->client->pers.pl_state = PL_SPECTATOR;
		gi.linkentity (other);
...
void droptofloor (edict_t *ent)
{
	...
	dest[2] -= 128; //QW// was VectorAdd(ent->s.origin, v, dest)
