...
int	num_clients	= 0;
void Spec(edict_t *ent, pmenu_t *menu);//RAV
...
void PutClientInServer (edict_t *ent)
{
	...
	if(!SelectSpawnPointRAV (ent, spawn_origin, spawn_angles))
	{
		...
		ent->solid = SOLID_NOT;
		ent->clipmask = 0;
		ent->client->pers.pl_state = 3;//force to try again
...
void ClientUserinfoChanged (edict_t *ent, char *userinfo)
{
	...
	if (ent->client->pers.netname == NULL)
	{
...
///* JSW - handled above^^^^^
	...
	{
		// check for 0 pingers
		if(!(ent->bot_client) && ent->client->pers.pl_state !=0)
		{   
...
//*/
	...
	//
	if(!ent->bot_client && ent->client->pers.pl_state == 1)
	{
	...
	if(ent->client->pers.pl_state == 1 && ent->client->resp.ctf_team > 0)
	{
...
qboolean ClientConnect (edict_t *ent, char *userinfo)
{
	...
	ent->client->pers.in_game = false;
	//RAV
	ent->client->pers.pl_state = 0; //spec
	//JSW
...
//end
	...
	{
	if((ucmd->msec == (int)speed_bust->value) && (ent->client->pers.pl_state ==1))
	...
	if((ent->client->resp.speed_cheat > 100) && (ent->client->pers.pl_state < 5)) //100 times over the speed limit!
	{
...
void ClientBeginServerFrame (edict_t *ent)
{
	...
	if(!ent->flags & FL_SPECIAL && ent->bust_time != 0 && !ent->client->resp.spectator &&
		ent->bust_time < level.time && ent->client->pers.pl_state < 5)
	{
	...
	{
		if(ent->client->pers.pl_state == 1)//playing in game
			ent->client->pers.pl_state = 2;//set to warmup
	}
	...
	if(ent->client->pers.pl_state == 0  && !ent->client->pers.motd && spec_check->value)
		Spec_Check (ent);
	
	//RAV add a small check to be sure all playing players are damagable
	if(ctf->value && ent->client->resp.ctf_team < 1 && ent->client->pers.pl_state != 5 &&!ent->bot_client)
		ent->client->pers.pl_state = 0;
	...
	if((ent->client->pers.pl_state == 0 || ent->client->resp.spectator)
		&&(ent->client->ps.gunindex != 0))
	{
