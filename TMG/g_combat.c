...
//RAV
	int			rsave;
	static qboolean is_vamp;
	is_vamp = rune_has_rune(attacker, RUNE_VAMP);
//
	if (!targ->takedamage)
		return;
...
// Location based damage
	if ((!(dflags & DAMAGE_RADIUS)) && (damage_locate->value == 1) )
	{
		if (!OnSameTeam (targ, attacker))
			take = ApplyLocationalSystem (attacker, targ, point, mod, take);
		else if (OnSameTeam (targ, attacker) && !(dmflag & DF_NO_FRIENDLY_FIRE))
			take = ApplyLocationalSystem (attacker, targ, point, mod, take);
	}
//
...
//RAV
	if(match_state == STATE_COUNTDOWN || level.warmup)
	{
		take = 0;
		save = damage;
	}
	// respawn protection
	if (client && (client->respawn_framenum > level.framenum))
	{
		take = 0;
		save = damage;
	}
...
//team armor protect
	if (ctf->value && targ->client && attacker && attacker->client &&
		...
		targ != attacker && (dmflag & DF_ARMOR_PROTECT))
	{
		psave = asave = 0;
	}
	else
...
//
	...
	if (!(dflags & DAMAGE_NO_PROTECTION) && CheckTeamDamage (targ, attacker)
		&& (dmflag & DF_NO_FRIENDLY_FIRE)) //JSW FF fix
		return;
...
//
		...
		if (is_vamp )
		{
			if(((!ctf->value) && (attacker != targ)) ||
				((ctf->value)&& (attacker != targ)
				&& !OnSameTeam (attacker, targ)))
			{
				...
				if (get < 3)
					get = 3;
				if (targ->client && !targ->deadflag && attacker->health > 0)
				{
					attacker->health += (int)(get / 2);
					if (attacker->health > max_Vhealth->value)
						attacker->health = max_Vhealth->value;
				}
			}
		}
		//end
