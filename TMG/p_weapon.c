...
//ZOID
				...
				if (rune_has_rune(ent, RUNE_HASTE))
					gi.sound(ent, CHAN_ITEM, gi.soundindex("flipper/flppain1.wav"), 0.7, ATTN_NORM, 0);
...
void Weapon_RocketLauncher (edict_t *ent)
{
	...
	Weapon_Generic (ent, 4, rocket_wait->value, 50, 54, pause_frames, fire_frames, Weapon_RocketLauncher_Fire);
}
...
//RAV
	if(voosh->value)
		Weapon_Generic (ent, 3, railwait->value, 56, 61, pause_frames, fire_frames, weapon_railgun_fire);
	else
//
		Weapon_Generic (ent, 3, railgun_wait->value, 56, 61, pause_frames, fire_frames, weapon_railgun_fire);
...
void Weapon_BFG (edict_t *ent)
{
	...
	Weapon_Generic (ent, 8, bfg_wait->value, 55, 58, pause_frames, fire_frames, weapon_bfg_fire);
}
